...
A **statistical investigative** question anticipates variability—that different people/items can yield different values—and can be answered with a measurable quantity you can collect (e.g., a count, percent, or average for a defined group and time).

* **Variability**: natural differences in values across people/items or over time (e.g., minutes read, flavors chosen).

* **Measurable quantity**: a clearly defined thing you can count/measure with a unit (e.g., “% of students who…” or “average minutes of…”).

In this lesson, we’ll practice telling investigable questions from vague ones, and make our first bar chart.
...
* Rewrite a vague question so it specifies what is measured (and unit), who it’s about, and when.
...
quiz(
  question("Which is *investigable with data*?",
    answer("What’s the best candy?", correct = FALSE, message = "This is an opinion question. 'Best' depends on personal taste. Data can’t prove one candy is best for everyone."),
...
Use this frame if helpful: “What **percent/number** of **[group] [did X / chose Y]** in **[time frame]**?”
...
**🔎 Challenge: Tinker with the bar chart and observe what changes.**  
...
answer("We’re counting a different variable, but the yes/no counts happen to be the same here, so the bars look the same (only the x label changes).", correct = TRUE),
...
answer("It’s measured in minutes.", correct = FALSE),
allow_retry = TRUE
)
)
...
textAreaInput("ethics_txt", "If you only survey friends sitting near you, what sampling bias might that introduce, and how could you reduce it? Write 2–3 sentences.", rows = 4)
```
